...
(define (im-enroll server-address port)
  ...
  (set! whoiam (cadr (string-split (number->string (current-second)) ".")))
  (let ((msg ""))
         ...
         (begin
          ...
          (cond ((string=? "sorry" (xml-cdata "command" msg))
            ...
            (else
             ...
             (set! handle-msg-thread (make-thread handle-msg))
             (thread-start! handle-msg-thread))))))))
        ...
        (begin
         (cond
           ...
           ((string=? "client-list" (xml-cdata "command" msg))
            (update-client-list (string-split (xml-cdata "data" msg))))
...
(define (close-connection)
  ...
  (display (format #f "Closing down message handling thread.~%")) ;LOGGING
  (thread-terminate! handle-msg-thread)
